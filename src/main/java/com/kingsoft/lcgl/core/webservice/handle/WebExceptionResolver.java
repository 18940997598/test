...
public class WebExceptionResolver extends AbstractHandlerExceptionResolver {
	...
	protected ModelAndView doResolveException(HttpServletRequest request,
        ...
        if(ex instanceof RestParamsException || ex instanceof UnexpectedTypeException) {
            ...
            if(((TokenException) ex).getResponse().getErrorCode() == 3){
                httpStatus = 408;
            }else {
                httpStatus = 409;
            }
            response.setStatus(httpStatus);
            commonResponse.put("errorCode", ((TokenException) ex).getResponse().getErrorCode());
            commonResponse.put("errorMsg", ((TokenException) ex).getResponse().getErrorMsg());
        }
        else if(ex != null) {
            httpStatus = 500;
            response.setStatus(httpStatus);
            commonResponse.put("errorCode", ExceptionMsg.SYSTEM_ERROR);
            commonResponse.put("errorMsg", ExceptionMsg.getRestExceptionResponse(ExceptionMsg.SYSTEM_ERROR).getErrorCode());
        }
        if(ex != null) {
